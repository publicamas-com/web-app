...
import { CategoryElement, ICategory } from '@/types/api/category.type';
import { useParams } from 'next/navigation';
import { callGetCategoryBySlug } from '@/client/apiClient';

const sortOptions = [
  { name: 'Más populares', href: '#', current: true },
  { name: 'Mejores puntuados', href: '#', current: false },
  { name: 'Nuevos', href: '#', current: false },
  { name: 'Más barato', href: '#', current: false },
  { name: 'Más caro', href: '#', current: false },
];
...
const ListLayout: React.FC<ListLayoutProps> = ({ children }) => {
  ...
  const [subCategories, setSubCategories] = useState<CategoryElement[]>([]);
  const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false);
  const [category, setCategory] = useState<ICategory>();
  useEffect(() => {
    if(categoryName) {
      callGetCategoryBySlug(categoryName as string).then(res => {
        debugger;
        setCategory(res)
                ...
                <ul role="list" className="px-2 py-3 font-medium text-gray-900">
                  {subCategories.map((category) => (
                    <li key={category.name}>
                      <a href={`/listado/${category.slug}`} className="block px-2 py-3">
                        {category.name}
                      </a>
                    </li>
                  ))}
